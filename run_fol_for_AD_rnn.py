...
def main(args, visualize=False):
    ...
    all_track_files = sorted(glob.glob(os.path.join(args.track_dir, '*.txt')))
    ...
    for video_idx, track_file in enumerate(all_track_files):
        try:
            ...
            while current_frame_id <= video_len:#track_data[-1][0]:
                ...
                if (current_frame_id < video_len):
                    flow_id = current_frame_id - 1
                else:
                    flow_id = current_frame_id - 2
                # print(video_len)
                # print(flow_id)
                flow_file = os.path.join(flow_folder, 
                                        str(format(flow_id,'06'))+'.flo')
